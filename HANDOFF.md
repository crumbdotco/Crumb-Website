...
- Review fix round 2 (2026-09-14): nine PR review findings fixed, all with tests and a class guard - see implementation-notes.md "B6c review-to-zero fix round (PR #21, second pass)" for the full list (anon-key RPC client rename, structured server-side error logging, compensating re-ban on partial unban, first-statement requireAdmin() guard extended to `'use server'` actions files, shared REPORT_PAGE_SIZE/AUDIT_HISTORY_LIMIT constants, strict ISO-8601 moderation cursor, a report status filter, and Vercel-preview-aware admin session origins). Full gate green: `npx jest` 33/468, `npx tsc --noEmit` unchanged pre-existing 7 errors only, `npx eslint` clean on changed files, `npx next build` succeeds.
